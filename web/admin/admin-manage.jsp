    ...
    <div class="bk-con-message message-section" id="iframe_box">
        ...
        <form id="myform" action="AdminServlet?flag=deletesAdmin" method="post">
            <div class="operation  mb15">
                ...
                <a class="btn button_btn bg-deep-blue" href="admin/admin-add.jsp">
                    <i class="iconfont"></i>
                    &nbsp;添加
                </a>
            </div>
            ...
            <div class="page_content clearfix mb15 table-module ">
                <table class="gallery table table_list table_striped table-bordered " id="">
                    ...
                    <c:forEach var="admin" items="${adminList}">
                        <tr>
                            <td><label><input type="checkbox" id="id" name="id" value="${admin.id}" class="ace"><span
                            ...
                            <td>
                                <a title="${admin.note}">${admin.noteShow }</a>
                            </td>
                            <td>
                                <fmt:formatDate value="${admin.editDate }" pattern="yyyy-MM-dd HH:mm:ss"/>
                            </td>
                            <td>
                                <c:if test="${admin.state==1}">
                                    <a href="AdminServlet?flag=lockAdmin&id=${admin.id}&pageIndex=${page.pageIndex }"
                                       onclick="return confirm('确定锁定麼?')">锁定</a>
                                </c:if>
                                <c:if test="${admin.state==2}">
                                    <a href="AdminServlet?flag=UnlockAdmin&id=${admin.id}&pageIndex=${page.pageIndex }"
                                       onclick="return confirm('确定解锁麼?')">解锁</a>
                                </c:if>

                                |<a href="AdminServlet?flag=getupdateAdmin&adminName=${admin.adminName}">修改</a>
                                |<a href="AdminServlet?flag=deleteAdmin&id=${admin.id}&pageIndex=${page.pageIndex }"
                                    onclick="return confirm('确定删除麼?')">删除</a>
                            </td>
...
<script>
    function subForm(pageIndex) {
        window.location.href = "AdminServlet?flag=manage&pageIndex=" + pageIndex;
    }
