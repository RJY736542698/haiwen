...
<div class="Bombbox-info Tab-Module">
    ...
    <div class="tab-box tabcontent">
        <form action="RoleServlet" method="post" name="myform">
        <div class="tab-conent prompt_style active">
            <input type="hidden" value="roleModify" name="flag" >
            <ul class="clearfix add_style">
                <li class="cell-item">
                    <label class="cell-left label_name" style="font-size: 12px;color:#8FA0AC;" >用户账号:</label>
                    <div class="cell-right">
                        <input type="hidden" name="id" value="${roleAdmin.id}">
                        <input type="text" style="font-size:12px;color:#8FA0AC;" class="col-xs-6 col-lg-12"
                                name="adminName"  value="${roleAdmin.adminName}"  readonly>
                    </div>
                </li>
                <li class="cell-item">
                    <label class="cell-left label_name" style="font-size: 12px;color:#8FA0AC;">用户角色:</label>
                    <input type="hidden" id="roleAdminName" name="roleAdminName" value="${roleAdmin.roleName}">
                    <div class="cell-right" style="font-size: 10px">
                        <table>
                            <c:forEach var="role" items="${roleList}">
                            <tr>
                                <td>
                                    <input type="radio" name="roleName" value="${role.roleName}">${role.roleName}
                                </td>
                            </tr>
                            </c:forEach>
                        </table>
            ...
            <div class="buttonstyle">
                ...
                <button id="form_btn" class="btn padding10 bg-deep-blue"   onclick="return confirm('确定修改吗?')">修改角色</button>
            </div>
        </div>
        </form>
    </div>
...
<script>
            ...
            for(var i=0;i<roleName.length;i++){
                if(roleAdminName.value==roleName[i].value){
                    roleName[i].checked=true;
                }
            }
