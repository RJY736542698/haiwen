    ...
    <div class="tab-box tabcontent">
        <form action="CateServlet" id="myForm" method="post" name="myform">
            <div class="tab-conent prompt_style active">
                <input type="hidden" value="smallCateupdate" name="flag">
                <input type="hidden" value="${cate.id}" name="smallCateId">
                <ul class="clearfix add_style">
                    <li class="cell-item">
                        <label class="cell-left label_name" style="font-size: 12px;color:#8FA0AC;">父级分类:</label>
                        <div class="cell-right">
                            ...
                            %>
                            <select id="bigCateId" name="parentId">
<%--                                <option selected="selected">--请选择所属一级分类</option>--%>
                                <c:forEach var="bigcate" items="${cateList}">
                                    <c:if test="${bigcate.id==cate.parentId}">
                                        <option selected value="${bigcate.id}">${bigcate.cateName}</option>
                                    </c:if>
                                    <c:if test="${bigcate.id!=cate.parentId}">
                    ...
                    <li class="cell-item">
                        ...
                        <div class="cell-right">
                            <input type="text" style="font-size:12px;color:#8FA0AC;" class="col-xs-6 col-lg-12"
                                   ...
                                   id="cateName" name="cateName" value="${cate.cateName }">
                            <span id="iscateName"></span>
                            <label class="validate_info" style="font-size:12px;color:#8FA0AC; margin-left: 70px" id="cateName_msg"
                                   >长度 2-20 位非空白字符</label>
                        </div>
                    </li>
                    <li class="cell-item">
                        ...
                        <div class="cell-right">
                            <textarea name="des" id="des"
                                      class="textarea col-xs-4 col-lg-12 height200">${cate.des}</textarea>
                        </div>
                    </li>
                </ul>
                <div class="buttonstyle">
                    <label id="result_msg" class="result_msg"> ${msg}</label>&nbsp;&nbsp;
                    <button id="form_btn" class="btn padding10 bg-deep-blue" onclick="return confirm('确定修改吗?')">保存修改
                    </button>
                </div>
            </div>
        </form>
...
<script>
    document.getElementById("form_btn").addEventListener("click", function (event) {
        ...
        var parentId=document.getElementById("bigCateId")
        var b = checkCateName();
        // 检查输入字段是否为空值
        if (cateName.value.trim() === "" || parentId.value.trim()==="" ) {
            alert("请填写所有必填字段！");
        }
        else if(b!=true){
            alert("修改失败，请按规定修改分类信息！");
        } else {
            // 提交表单
            form.submit();
        }
